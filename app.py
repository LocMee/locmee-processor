...
def check_password():
    ...
    with col_pwd2:
        ...
        # --- JAVASCRIPT PARA FOCO AUTOMÁTICO NO CAMPO DE SENHA ---
        components.html("""
            <script>
                // Função para focar no input de senha assim que o elemento carregar na tela
                function focarSenha() {
                ...
                // Tenta imediatamente e também após pequenos intervalos para garantir o carregamento do Streamlit
                setTimeout(focarSenha, 100);
                setTimeout(focarSenha, 400);
                setTimeout(focarSenha, 800);
            </script>
        """, height=0)
        ...
        if termo_busca:
            ...
            if len(df_busca) > 0:
                ...
                for idx, row in df_busca.head(10).iterrows():
                    ...
                    telefone = achar_valor/(["telefones", "telefone", "celular", "whatsapp", "fone"]) if 'acha_valor' else achar_valor(["telefones", "telefone", "celular", "whatsapp", "fone"])
